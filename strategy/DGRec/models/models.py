...
from DGRec.models.layers import DGRecLayer
...
class DGRec(BaseGraphModel):
    ...
    def get_recommendations(self, active_users, k, users_items_matrix):
        ...
        scores = self.get_score(h, active_users)
        already_interacted_items = np.where(users_items_matrix[active_users])

        scores[already_interacted_items] = -float('inf')

        _, recommended_items = torch.topk(scores, k=k)
        recommended_items = recommended_items.cpu().numpy()

        final_recommendation_list = list(map(list, recommended_items))
